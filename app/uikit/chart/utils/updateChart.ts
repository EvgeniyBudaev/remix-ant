  ...
  if (variantChart === EChartVariants.Variant1) {
    chart.data(
      (data as TChartVariant1[]).map((value) => ({
        ...value,
        positiveAdjusted:
          value.positive.crypto &&
          Math.max(value.positive.crypto, minVisibleValue),
        negativeAdjusted:
          value.negative.crypto &&
          Math.min(value.negative.crypto, -minVisibleValue),
      }))
    );
    ...
    const getTypeFormat = (value: number, type: EAmount) => {
      if (type === EAmount.Crypto) {
        return (
          formatCryptoCurrency?.(Math.abs(value), banknote ?? "") ??
          Math.abs(value)
        );
      }
    ...
    const getAmount = (value: number, type: EAmount, status: EStatus) => {
      return `${getOperator(value, status)}${currencyUnit}${getTypeFormat(
        value,
        type
      )}`;
    };
    ...
    chart
      ...
      .style({ radius: 4 })
      .color("positiveAdjusted", () => getThemeColor("--color-accent-2"))
    ...
    chart
      ...
      .color("negativeAdjusted", () => getThemeColor("--color-accent-1"))
      .style({ radius: 4 })
      .adjust([{ type: "dodge", dodgeBy: "part", marginRatio: 1 }]);
  }
